...
public class Pkcs11Credential extends AbstractReloadablePkiCredential {
  ...
  public Pkcs11Credential(final Pkcs11Configuration configuration, final String alias, final char[] pin)
    ...
    this(configuration, alias, pin, (X509Certificate) null);
  }
  ...
  private synchronized void load() throws IllegalArgumentException, SecurityException {
    ...
    // Load the private key, and possibly the certificate(s) ...
    //
    PrivateKey pk = null;
    List<X509Certificate> certs = super.getCertificateChain();
    if (!certs.isEmpty()) {
  ...
  public PublicKey getPublicKey() {
    return Optional.ofNullable(this.getCertificate()).map(X509Certificate::getPublicKey).orElse(super.getPublicKey());
  }
